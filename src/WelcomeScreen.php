...
class WelcomeScreen {
	...
	public function custom_dashboard_welcome_panel(): void {
		...
		// Get all public post types (except attachments and CPT indexes) to create links to them.
		// Using publicly_queryable here is not suitable because of things like my blocks' plugin's "Shared content"
		// which is not publicly queryable but is available in the admin UI.
		$post_types = get_post_types(['public' => true], 'objects');
		$post_types = array_filter($post_types, function($def) {
			return $def->name !== 'attachment' && $def->name !== 'cpt_index';
		});
		...
		$include = apply_filters('doublee_welcome_screen_included_post_types', array_keys($post_types));
		...
		if(class_exists('WC_Subscriptions_Admin') && current_user_can('manage_woocommerce')) {
			$primary_links[] = [
		...
		// WooCommerce general orders
		if(class_exists('WC_Order') && current_user_can('manage_woocommerce')) {
			$primary_links[] = [
				'label' => 'Manage orders',
		...
		if(class_exists('WC_Box_Office_Product_Admin') && current_user_can('manage_woocommerce')) {
			$primary_links[] = [
				'label' => 'Manage event tickets',
				'url'   => admin_url('edit.php?post_type=event_ticket'),
