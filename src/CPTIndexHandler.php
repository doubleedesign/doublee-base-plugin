...
class CPTIndexHandler {
	...
	public function __construct() {
		...
		add_filter('template_redirect', [$this, 'redirect_to_real_archive_or_custom_link'], 20);
	...
	public function use_real_index_url_for_permalinks(string $url, WP_Post $post): string {
		...
		$redirect_url = $this->get_real_url($post->ID);
		if($redirect_url) {
			return $redirect_url;
		}
	 ...
	 * @return void
	 */
	public function redirect_to_real_archive_or_custom_link(): void {
		if(!$this->is_cpt_index()) return;

		$queried_object = get_queried_object();
		$real_url = $this->get_real_url($queried_object->ID);
	...
	/**
	 * If we hit the archive page but a custom redirect URL is set, redirect to that
	 * @return void
	 */
	public function maybe_redirect_archive(): void {
		if(!is_post_type_archive()) return;

		$post_type = get_query_var('post_type');
		$index_id = self::get_index_id_for_post_type($post_type);

		if(!$index_id) return;

		$custom_redirect = get_post_meta($index_id, 'alternative_redirect', true);
		if($custom_redirect) {
			wp_redirect($custom_redirect['url'], 301);
			exit;
		}
	}

	private function is_cpt_index(): bool {
		$queried_object = get_queried_object();
		if(!$queried_object) {
			return false;
		}
		...
		return $queried_object->post_type !== 'cpt_index';
	}

	protected function get_real_url($index_id): false|string|null {
		$custom_redirect = self::get_custom_redirect_url($index_id);
		if($custom_redirect) {
			return $custom_redirect;
		}

		$indexed_type = get_post_meta($index_id, 'indexed_post_type', true);
		if($indexed_type) {
			return get_post_type_archive_link($indexed_type);
		}

		return null;
	}

	private static function get_index_id_for_post_type(string $post_type) {
		$query = new \WP_Query(array(
			...
			'meta_query'     => array(
				array(
					'key'   => 'indexed_post_type',
					'value' => $post_type,
				),
			),
		));

		$indexes = wp_list_pluck($query->posts, 'ID');
		if(empty($indexes)) {
			return null;
		}
	...
	private static function get_custom_redirect_url($index_id): ?string {
		$custom_redirect = get_post_meta($index_id, 'alternative_redirect', true);
		if($custom_redirect) {
	...
	public function register_alternative_redirect_field(): void {
		acf_add_local_field_group(array(
			'key' => 'group_cpt_index_settings',
			'title' => 'Settings',
			'fields'   => array(
				array(
					'key'      => 'field_alternative_redirect',
					'label'        => 'Alternative Redirect URL',
					'name'         => 'alternative_redirect',
					'type'         => 'link',
					'instructions' => '"Indexes" exist to allow "archives" (which are pages that are auto-generated by WordPress and are generally not directly editable) to be treated like pages for features like "Related content". By default, an Index redirects to the standard "archive" page for the indexed post type. If you would like it to redirect to a different piece of content (for example, redirecting the Staff Profiles index to the About Us page), you can specify that here.',
				),
			),
			'location' => array(
				array(
	...
	public static function post_type_has_custom_redirect(string $post_type): bool {
		$index_id = self::get_index_id_for_post_type($post_type);
		if(!$index_id) return false;

		return !empty(self::get_custom_redirect_url($index_id));
	}
